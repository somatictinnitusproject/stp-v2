...
// GET /api/profile/replies?username=X&page=N
//
// Auth + access gate. Resolves username → user_id via
// service-role profile fetch, then loads paginated replies
// via the standard anon client.
...
export async function GET(request: Request) {
  ...
  const result = await getUserReplies(supabase, profile.id, page, 20)
