// /content/framework/phase-3/d3-release-intro.ts
// D.3 — Release Phase Introduction
// Verbatim member-facing copy from Document 8 Part D, section D.3.
// Daily focus: "Release before you retrain"
//
// No profile modifier. All jaw protocol members read this regardless of profile type.

import type { ReadingSection } from './types'

const d3: ReadingSection = {
  kind: 'reading',
  id: 'D3_release_intro',
  section: 'D.3',
  title: 'Release Phase Introduction',
  estimatedMinutes: 4,
  content: [
    {
      type: 'subhead',
      text: 'The Release Phase',
    },
    {
      type: 'p',
      text: 'The release phase has one job: reduce the chronic tension, compression, and movement dysfunction in your jaw and masticatory system that has been generating abnormal trigeminal input.',
    },
    {
      type: 'p',
      text: 'Before any strengthening or retraining work is introduced, the tissues need to be in a state where that work is productive. Loading a chronically tensioned jaw before the underlying tension has reduced does not build a stronger, more stable system — it reinforces the existing dysfunction under additional load.',
    },
    {
      type: 'p',
      text: 'Release comes first because the resistance phase only works on a foundation the release phase has built.',
    },
    {
      type: 'p',
      text: 'The exercises in this phase work through a specific sequence. Heat first, to prepare the tissue. Muscle release work next, targeting the temporalis, masseter, and pterygoids in order of accessibility. Joint and nerve work after that, addressing the TMJ and auriculotemporal nerve directly. Hyoid and suprahyoid last, resolving the tension at the junction between your jaw and neck.',
    },
    {
      type: 'p',
      text: 'Resting position retraining runs continuously throughout the day alongside all of this — not a session exercise but a persistent habit that reduces the rate at which tension rebuilds between sessions.',
    },
    {
      type: 'p',
      text: 'Work through the exercises in the order they are presented. The sequence is not arbitrary.',
    },
    {
      type: 'subhead',
      text: 'About These Sessions',
    },
    {
      type: 'p',
      text: 'As you work through the exercises for the first time, each one includes a full explanation of what it does and why. From your second session onward, each exercise shows a condensed version — your demonstration video and the key instructions only. The full explanation remains accessible if you want to return to it, but your daily sessions stay focused and efficient.',
    },
    {
      type: 'subhead',
      text: 'When to Progress to the Resistance Phase',
    },
    {
      type: 'p',
      text: 'You decide when you are ready to move on. The minimum before introducing resistance work is one to two weeks of consistent daily release sessions. The signals that indicate your tissues are ready:',
    },
    {
      type: 'list',
      items: [
        'Tenderness on masseter and temporalis self-palpation has reduced noticeably.',
        'The release work feels less effortful — tissues responding more readily to pressure.',
        'Jaw drift on mirror check has reduced or is less consistent.',
        'Release sessions are producing a clear sense of tissue softening rather than resistance.',
      ],
    },
    {
      type: 'p',
      text: 'You do not need all four. A clear pattern of improvement across most of them is sufficient.',
    },
    {
      type: 'p',
      text: 'Some members are ready closer to one week. Others with longer-standing dysfunction or significant disc pathology take longer. Both are normal. The tissue tells you — not the calendar.',
    },
    {
      type: 'acknowledge_prompt',
      text: 'I understand the purpose of the release phase and how to recognise when I’m ready to progress.',
    },
  ],
}

export default d3
